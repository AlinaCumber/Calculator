    ...
    public void ArabAndRomanCalc() throws Exception {
        ...
        while (true) {
            ...
            if (Arrays.asList(numRom).contains(a) && Arrays.asList(numRom).contains(b)) {
                ...
                int number = VerifyData.tryParseInt(a, -1);
                int numberTwo = VerifyData.tryParseInt(b, -1);

                if (number < 0 || numberTwo < 0 || number > 10 || numberTwo > 10) {
                    throw new Exception("Только одна система исчисления. и только от 1 до 10");
                }


                for (int i = 0; i < numAr.length; i++) {
                    for (int j = 0; j < numAr.length; j++) {
                        if (number == numAr[i] && numberTwo == numAr[j]) {
                            int result = MathCalc.Calc(number, numberTwo, op);
